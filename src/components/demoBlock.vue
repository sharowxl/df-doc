...
export default {
  ...
  computed: {
    ...
    blockClass () {
      return `demo-${this.$router.currentRoute.path.toLowerCase().split('/').pop()}`
    },
    iconClass () {
      return this.isExpanded ? 'df-icon-triangle-up' : 'df-icon-triangle-down'
    },
  ...
  watch: {
    isExpanded (val) {
      ...
      setTimeout(() => {
        this.scrollParent = document.querySelector('.page-container')
