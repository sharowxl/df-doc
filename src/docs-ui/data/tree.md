...
:::demo 声明属性`custom-icon`, 该属性是一个函数, 自带 4 个参数, 分别是 节点、节点数据、节点是否展开、节点是否是最终层节点, 根据这些状态判断要为此节点赋予何种图标, 在函数的最后`return`一个对象, 对象包含图标类型和图标名称, 目前我们 [图标库](#/component/icon) 支持的图标类型有`icon`型和`svg`型, 如果`return`的是一个空值, 则会自动使用原本的默认图标。
```html
<template>
...
:::demo 给 tree 传递函数`node-disabled-func`, 该函数自带两个参数, 节点的 node 和 data, 通过对节点数据的判断, 返回是否禁用, 与节点数据中的`disabled`同时生效。
```html
<template>
  <df-tree
    :data="data"
...
:::demo 传入属性`need-operations`以开启树节点更多操作，传入`set-operations`方法以配置某一节点处的按钮组，此属性是一个函数，参数是节点对象，通过业务判断，返回该节点的按钮组即可，若安徽的按钮组为空，那么此节点的操作按钮将被隐藏。当某一按钮被点击时，会外派事件`operation-click`，携带被点击处的信息。属性`hide-on-click`表示点击按钮后是否自动隐藏弹出框。支持传入`hidden-operations`、`loading-operations`或`disabled-operations`设置某些按钮的隐藏、禁用或加载，需要注意的是`hidden-operations`、`loading-operations`或`disabled-operations`必须通过 Vue 指定的 [数组更新检测](https://cn.vuejs.org/v2/guide/list.html#%E6%95%B0%E7%BB%84%E6%9B%B4%E6%96%B0%E6%A3%80%E6%B5%8B) 方法来改变值，否则将不能监听其变化。
```html
<template>
  <div>
    <df-tree
...
| need-operations | /*新增*/  是否需要配置节点更多操作按钮组 | boolean | — | false |
| set-operations | /*新增*/ 动态配置节点更多操作按钮组的名称 | Function(node) | — | — |
| hidden-operations | /*新增*/ 设置节点更多操作按钮组的隐藏 | Array | — | — |
| loading-operations | /*新增*/ 设置节点更多操作按钮组的 loading 状态 | Array | — | — |
| disabled-operations | /*新增*/ 设置节点更多操作按钮组的 disabled 状态 | Array | — | — |
| hide-on-click | /*新增*/ 是否在弹出节点更多操作按钮组后隐藏弹出框 | boolean          | — | true |
| stop-on-click-operation | /*新增*/ 是否在点击更多操作三个点时阻止点击事件的向上冒泡 | boolean          | — | true |
| show-tree-icon | /*新增*/ 是否显示树形控件的图标 | boolean          | — | true |
| node-disabled-func | /*新增*/ 通过回调函数的形式控制某些节点的禁用状态 | function          | — | — |
| custom-icon | /*新增*/ 通过回调函数的形式控制节点的图标展示, 函数自带 4 个参数, 分别是 节点、节点数据、节点是否展开、节点是否是最终层节点  | function          | — | — |
...
| setNodeExpandStatus     | 设置某个节点的展开状态  | (nodeKey, expandStatus) 接收两个参数，1. 节点的 key 2. 节点的展开状态，true 为展开，false 为收起 |
| setAllExpandStatus     | 设置全部节点的展开状态  | (expandStatus) 接收一个参数，全部节点的展开状态，true 为展开，false 为收起 |
...
| operation-click  | /*新增*/ 使用节点更多操作按钮组时，触发任一操作后触发（参数：操作处的节点、操作名称、操作在操作组中的下标） | node, operation, operationIndex |
