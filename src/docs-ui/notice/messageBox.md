...
```html
<template>
  <df-button type="text" @click="open">点击打开 Message Box</df-button>
</template>
...
:::demo 配置`showLink`为`true`，并设置`linkText`和`linkCallback`即可使用。
```html
<template>
  <df-button type="text" @click="open">点击打开 Message Box</df-button>
</template>
...
:::demo 配置属性 `needDrag` 为 `true` 即可开启拖拽，注意在非全屏模式下，拖拽才有效
...
:::demo 配置属性 `needFullscreenToggle` 为 `true` 即可开启全屏/退出全屏切换。
...
如果你完整引入了 DfUI Vue.prototype 添加如下全局方法：$msgbox, $alert, $confirm 和 $prompt。因此在 Vue instance 中可以采用本页面中的方式调用 `MessageBox`。调用参数为：
- `$msgbox(options)`
...
| needDrag | *（新增）* 是否开启 MessageBox 的拖拽，注意在非全屏模式下，拖拽才有效 | boolean    | — | false |
| needFullscreenToggle | *（新增）* 是否开启 MessageBox 的全屏/退出全屏切换 | boolean    | — | false |
| showLink | *（新增）* 是否开启 MessageBox 的链接跳转 | boolean    | — | false |
| linkText | *（新增）* 开启 MessageBox 的链接跳转时，链接的文本内容 | string    | — | — |
| linkCallback | *（新增）* 开启 MessageBox 的链接跳转时，链接被点击后的回调函数 |  function    | — | false |
